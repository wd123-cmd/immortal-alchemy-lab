...
# -------------------------------
# 1. IMAGE DECOMPILER ENGINE
# -------------------------------
@st.cache_resource
def load_ocr():
    # This caches the 'brain' so it only loads once
...
def decompile_screenshot(image_file, herb_list):
    ...
    # Finds numbers (quantities) and matches nearby text to Herb Names
    for i, (bbox, text, prob) in enumerate(results):
        val = text.lower().replace('x', '').strip()
        
        if val.isdigit():
            quantity = int(val)
            # Look at the next 2 detected text blocks for a name match
            for j in range(1, 3):
                if i + j < len(results):
                    potential_name = results[i+j][1].lower().strip()
                    for herb in herb_list:
                        if herb.lower() in potential_name:
...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        border-radius: 8px 8px 0 0; padding: 12px 15px; color: #8b949e;
    }
    ...
    .benefit-tag {
        ...
        font-size: 0.75rem; font-weight: 600; margin-right: 4px; margin-top: 4px;
    }
...
with tab2:
        ...
        if st.button("✨ Scan Screenshot"):
            with st.spinner("Decoding materials..."):
                ...
                if found:
                    ...
                    st.success(f"Decompiled {len(found)} items!")
                    st.rerun()
                else:
                    st.error("No herbs detected. Try a clearer image.")
    ...
    h_search = st.text_input("🔍 Manual Search...", placeholder="Herb name...").lower()
    cols = st.columns(2)
...
with tab1:
    p_query = st.text_input("🔍 Search Recipes...", placeholder="Name or effect...").lower()
    ...
    if craftable:
        for p in sorted(craftable, key=lambda x: x['qi'], reverse=True):
            ...
            tags = f'<span class="benefit-tag dur-tag">{"Permanent" if is_perm else "Temporary"}</span>'
...
{totals}
        ...
        st.info("No craftable items. Add ingredients in the next tab.")
